    ...
    try:
        ...
        if og_image and og_image.get('content'):
            ...
            if image_url.startswith('//'):
                ...
                # Try to construct full URL from article URL
                from urllib.parse import urlparse
                parsed = urlparse(url)
                image_url = f"{parsed.scheme}://{parsed.netloc}{image_url}"
        else:
            # Try to find featured image in article
        ...
        if og_description and og_description.get('content'):
            ...
            if article_content:
                ...
                if first_p:
                    description = first_p.get_text().strip()[:200]  # Limit to 200 chars
...
def enhance_article_data(titles, urls, imageURLS, blurbs, authors=None, max_enhance=10, enhance_all=False):
    ...
    This ensures all article cards are fully populated with complete data.
    Can either enhance only missing data or fetch complete data for all articles.
    
    Args:
        titles: List of article titles
        urls: List of article URLs
        imageURLS: List of image URLs
        blurbs: List of article descriptions/blurbs
        authors: Optional list of authors
        max_enhance: Maximum number of articles to enhance (default: 10)
        enhance_all: If True, fetch complete data for all articles, not just missing ones
        
    Returns:
        Tuple of enhanced (titles, urls, images, blurbs, authors) lists
    """
    ...
    # Enhance articles - either all or just missing data
    enhanced_count = 0
    ...
    for i in range(articles_to_process):
        ...
        # Check if we need to enhance this article
        if enhance_all:
            # Always enhance when enhance_all is True
            should_enhance = True
        else:
            # Only enhance if missing data
        ...
        if should_enhance:
            ...
            # Update data - prefer fetched data if enhance_all, otherwise only fill missing
            if enhance_all:
                # Always use fetched data if available (more complete)
                if title:
